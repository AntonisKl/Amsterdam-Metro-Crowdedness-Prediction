...
gvb_dfs_final, covid_df, covid_measures, holiday_df, vacations_df, knmi_forecast, events = None, None, None, None, None, None, None


def preprocess():
    global covid_measures, vacations_df, events, covid_df, holiday_df, knmi_forecast, gvb_dfs_final

    # Global variables are written only once
    if gvb_dfs_final is not None:
        return

    herkomst_2020, bestemming_2020, herkomst_2021, bestemming_2021, knmi_obs, knmi_preds, covid_measures, covid_df_raw, holidays_data_raw, vacations_df, events = read_data()
...
def train_and_predict():
    ...
    config_stringency = request.args.get('useStringency', type=bool)
    config_measures = request.args.get('useMeasures', type=bool)
    ...
    data_splits, X_train_splits, y_train_splits, X_validation_splits, y_validation_splits, X_test_splits, y_test_splits, X_predict_dfs = split_data_for_modelling(
        gvb_dfs_final, covid_df, covid_measures, holiday_df, vacations_df, knmi_forecast, events, features)
