...
def preprocess():
    global covid_measures, df_covid_filtered, vacations_df, events, covid_df, holiday_df, knmi_forecast, gvb_dfs_final, covid_measures_columns
    ...
    covid_measures_columns = list(covid_measures.columns.values)
    ...
    gvb_dfs_merged, covid_df, holiday_df, knmi_forecast = preprocess_data(herkomst_2020, bestemming_2020, herkomst_2021,
                                                                          ...
                                                                          covid_measures, df_covid_filtered,
                                                                          covid_df_raw,
                                                                          holidays_data_raw,
                                                                          vacations_df, events)
...
def train_and_predict():
    ...
    data_splits, X_train_splits, y_train_splits, X_validation_splits, y_validation_splits, X_test_splits, y_test_splits, X_predict_dfs = split_data_for_modelling(
        gvb_dfs_final, covid_df, covid_measures, df_covid_filtered, holiday_df, vacations_df, knmi_forecast, events,
        features)
