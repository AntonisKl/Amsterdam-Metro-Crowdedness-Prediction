...
def prepare_data(env, resono_data_dir, file_name, freq, predict_period, start_prediction, n_samples_day, Y_names, 
    ...
    def listToStringWithoutBrackets(Y_names):
    
        return str(Y_names).replace('[','').replace(']','')
    
    datelist = pd.date_range(start_prediction - timedelta(days=4), start_prediction).strftime('%Y/%m/%d').tolist()
    
    query = "select measured_at, location_id, location_name, total_count from ingested.resono where location_desc IN (" + listToStringWithoutBrackets(Y_names) + ") AND measured_at::date between '%s' and '%s'" %(str(datelist[0]), str(datelist[-1]))
    
    resono_pgres = pd.read_sql_query(query, con=engine_azure)
    
    def preprocess_resono_pgres (df):
            
        df = df.set_index(['measured_at','location_name']).groupby([pd.Grouper(level='measured_at', freq='H'), pd.Grouper(level='location_name')]).sum().reset_index()
        
        df['measured_at'] = df['measured_at'].astype('datetime64[ns]')
        df['Hour'] = df['measured_at'].dt.hour
        df['Date'] = df['measured_at'].dt.date
        df = df.rename(columns={'location_name':'Location','total_count':'Visits'})[['Location','Date','Hour','Visits']]
        
        return df
    
    resono_pgres = preprocess_resono_pgres(resono_pgres)
    ...
    resono_df_raw = pd.concat([resono_pgres, resono_df_raw[['Location','Date','Hour','Visits']]])
    ...
    knmi_obs_minio = spark.read.format("json").option("header", "true").load("s3a://knmi-knmi/topics/knmi-observations/2021/06/*/*", sep = ";", schema=knmi_obs_schema)
    knmi_obs_df_raw = knmi_obs_minio.toPandas()
    knmi_pred_minio = h.get_knmi_preds()
