...
def get_crowd_levels(df, Y_name, thresholds):
    '''
    ...
    th = thresholds[Y_name]
    df[Y_name] = pd.cut(df[Y_name], bins = th, labels = labels)
...
def create_SVM_models(df, min_samples = 1, nu = 0.2, gamma = 0.1):
    """
    ...
    # nu: value for nu parameter (default is 0.2) 
    # gamma: value for gamma parameter (default = 0.1)
...
def train_model_linear_regression(df_X_train, df_y_train):     
...
def test_model_linear_regression(model, df_X_test):
    """
...
def train_model_ordinal_regression(df_X_train, df_y_train, use_sample_weights = False, use_smote = False):     
...
def perform_smote(df_X_train, df_y_train):
    ...
    y_train = df_y_train.iloc[:, 0].astype('category')

    # Only oversample if there are at least 3 samples
    if all(pd.Series(y_train).value_counts() >= 3):
        ...
        print("SMOTE not used: not enough samples")
...
def prepare_backtesting(start_test, predict_period, freq, df, Y_name, cols_unknown, n_samples_week, target,
                       y_scaler):
    '''
    ...
    for col in cols_unknown:
        df_X_predict_bt[col] = df_X_predict_bt[col].shift(n_samples_week)
